...
test.cb('should not delete a fork that has more branches', function(t) {
  var mock = lib.mock({
    getBranches: function(args) {
      if (args.owner === lib.USER.login) return ['foo'];
      return [];
    },
  });
...
test.skip('should delete a fork that has more branches, but all at upstream branch tips', function(t) {
  var mock = lib.mock({
    getBranches: function(args) {
      ...
      return [{ name: 'master', commit: lib.COMMIT_A }, { name: 'baz', commit: lib.COMMIT_B }];
    },
    delete: true,
  });
...
test.cb('should delete a fork that has all branches at upstream branch tips', function(t) {
  var mock = lib.mock({
    getBranches: function(args) {
      if (args.user === lib.USER.login)
        return [{ name: 'master', commit: lib.COMMIT_A }, { name: 'foo', commit: lib.COMMIT_B }];
      return [{ name: 'master', commit: lib.COMMIT_A }, { name: 'baz', commit: lib.COMMIT_B }];
    },
    delete: true,
  });
...
test.cb('should delete a fork that has branches behind', function(t) {
  var mock = lib.mock({
    getBranches: function(args) {
      if (args.owner === lib.USER.login)
        return [{ name: 'master', commit: lib.COMMIT_A }, { name: 'foo', commit: lib.COMMIT_B }];
      return [{ name: 'master', commit: lib.COMMIT_C }, { name: 'foo', commit: lib.COMMIT_C }];
    },
  ...
  removeGithubForks(mock.present, function() {
    lib.check(t, mock.calls(), [
      ...
      [
        'compareCommits',
        { owner: lib.AUTHOR.login, repo: 'upstream-lib', base: lib.COMMIT_B.sha, head: 'master' },
      ],
      [
        'compareCommits',
        { owner: lib.AUTHOR.login, repo: 'upstream-lib', base: lib.COMMIT_B.sha, head: 'foo' },
      ],
      [
        'compareCommits',
        { owner: lib.AUTHOR.login, repo: 'upstream-lib', base: lib.COMMIT_A.sha, head: 'master' },
      ],
      ['delete', { owner: lib.USER.login, repo: 'fork1', url: undefined }],
    ]);
    t.end();
...
test.cb('should not delete a fork that has diff that is too long to generate', function(t) {
  var mock = lib.mock({
    getBranches: function(args) {
      if (args.owner === lib.USER.login) return [{ name: 'master', commit: lib.COMMIT_A }];
